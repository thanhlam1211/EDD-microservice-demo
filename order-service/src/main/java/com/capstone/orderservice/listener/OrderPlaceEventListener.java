package com.capstone.orderservice.listener;
    ...
    public void handleOrderPlacedEvent(OrderPlaceEvent event) {
        ...
        try {
            Observation.createNotStarted("notification-topic", this.observationRegistry).observeChecked(() -> {
                CompletableFuture<SendResult<String, OrderPlaceEvent>> future = kafkaTemplate.send("notificationTopic",
                        new OrderPlaceEvent(event.getOrderNumber()));
